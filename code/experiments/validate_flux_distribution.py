"""validate_flux_distribution.py -- ITEM 2 validation: leakage checks then the
full comparison, on the frozen split.
...
FEATURES_CSV = os.path.join(SCRIPT_DIR, "flux_distribution_features.csv")
RESULTS_PATH = os.path.join(SCRIPT_DIR, "flux_distribution_results.json")

RANDOM_SEED = 42
N_BOOTSTRAP = 2000
N_CV = 5
NEW = ["in_skew", "in_kurt", "out_skew", "out_kurt", "skew_diff", "kurt_diff",
...
def main():
    ...
    out = {}
    probs = {}
    for name, Xv in (("base", Xb), ("with_flux_stats", X)):
    ...
    m_, lo, hi = boot(y[te], probs["base"], probs["with_flux_stats"])
    clears = lo > 0
